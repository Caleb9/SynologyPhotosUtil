...
[<Fact>]
let ``Full sunshine scenario: list owned album containing 4 photos`` () =
    async {
        (* Arrange *)
        let args =
            [| "--address"
        ...
        let sendRequestStub (request: HttpRequestMessage) =
            task {
                ...
                return
                    ...
                    | getPersonalSpaceFolderForPhoto4 when
                        getPersonalSpaceFolderForPhoto4
                        |> matches
                            "http://ds.address:5000/webapi/entry.cgi/SYNO.Foto.Browse.Folder"
                            actualContentString
                            [ "_sid=fake_sid"
...
[<Fact>]
let ``Full sunshine scenario: list person album containing 3 photos`` () =
    async {
        (* Arrange *)
        let args =
            [| "--address"
...
[<Fact>]
let ``Full sunshine scenario: list shared_with_me album containing 3 photos`` () =
    async {
        (* Arrange *)
        let args =
            [| "--address"
